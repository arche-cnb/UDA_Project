import { useState } from 'react'
import AuthModal from './AuthModal'

export default function Header({ user, onLogin, onLogout }) {
  const [showAuth, setShowAuth] = useState(false)

  return (
    <header style={{
      height: '56px',
      background: '#16161a',
      borderBottom: '1px solid #2a2a32',
      display: 'flex', alignItems: 'center',
      padding: '0 28px',
      position: 'sticky', top: 0, zIndex: 50,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2.5">
          <rect x="2" y="5" width="20" height="14" rx="3"/>
          <path d="M7 15V9h4a3 3 0 0 1 0 6H7"/>
        </svg>
        <span style={{ fontSize: '16px', fontWeight: 700, color: '#e8e8ea', letterSpacing: '-0.02em' }}>
          ParkManager
        </span>
      </div>

      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 12 }}>
        {user ? (
          <>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', marginRight: 8 }}>
              <span style={{ fontSize: 12, color: '#e8e8ea', fontWeight: 700 }}>{user.email}</span>
              <span style={{ fontSize: 11, color: '#6b7280' }}>{user.role}</span>
            </div>
            <button onClick={() => onLogout()} style={{ background: 'transparent', border: '1px solid #2a2a32', color: '#6b7280', padding: '8px 10px', borderRadius: 8, cursor: 'pointer' }}>Esci</button>
          </>
        ) : (
          <>
            <button onClick={() => setShowAuth(true)} style={{ background: 'linear-gradient(135deg,#10b981,#059669)', border: 'none', color: '#fff', padding: '8px 10px', borderRadius: 8, cursor: 'pointer' }}>Accedi / Registrati</button>
          </>
        )}
      </div>

      {showAuth && <AuthModal onClose={() => setShowAuth(false)} onLogin={(u) => { onLogin(u); setShowAuth(false) }} />}
    </header>
  )
}